...
import static com.application.test.worldtime.gui.MainActivity.m_IsClosingApp;
import static com.application.test.worldtime.gui.MainActivity.m_IsHomeTimezoneSet;

/**
 * Created by Muzammil on 17/01/2017.
 */
...
public class HomeFragment extends Fragment
{
    ...
    public TimezoneDbHelper GetDbHelper()
    {
        return mTimezoneDbHelper;
    }
...
//                        cursor.close();
                        ...
                        for(int index=0; index<mList.size(); index++)
                        {
                            ...
                            if(country_id == item.getId())
                            {
                                ...
                                if(mList.get(index).getIsHomeTimezone() == true)
                                    MainActivity.m_IsHomeTimezoneSet = false;
        ...
        if(mTimezoneDbHelper == null)
            mTimezoneDbHelper = TimezoneDbHelper.getInstance(getContext());

        mChkUseCurrentTime = ((CheckBox) rootView.findViewById(R.id.chkUseCurrentTime));
...
//        });
        ...
        PrepareDatabase_Async asyncTask = new PrepareDatabase_Async(getContext());
        asyncTask.execute();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        {
            case R.id.add_menu:
                String name = getResources().getResourceEntryName(R.id.add_menu);
                Intent intent = new Intent(getContext(), add_timezone.class);
                startActivityForResult(intent, 10/*, ActivityOptions.(this).toBundle()*/); // requestCode = 10
                m_IsClosingApp = false;
                return super.onOptionsItemSelected(item);
            case R.id.action_settings:
                return true;
            case 16908332/*R.id.navigation_drawer*/:
                name = getResources().getResourceEntryName(16908332);
//                View main_activity = getActivity().findViewById(R.id.drawer_layout)
//                main_activity.d
                Toast.makeText(getContext(), "Navigation Drawer", Toast.LENGTH_SHORT).show();
                return true;
            default:
                Toast.makeText(getContext(), "Unrecognized Action", Toast.LENGTH_SHORT).show();
                return super.onOptionsItemSelected(item);
        }
    }
        ...
        if(requestCode == 10)
        {
            ...
            if(mList.size() > 0)
            {
                for (int index = 0; index < mList.size(); index++)
                {
                    ...
                    {
                        Toast.makeText(getContext(), "Timezone Already Added !!", LENGTH_SHORT);
                        return;
                    }
                }
            }
